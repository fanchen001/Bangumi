 ...
 */
public class K8dyImpl implements IVideoParser {

    private CcyImpl impl = new CcyImpl(K8dyService.class.getName());
...
//    }
    ...
    public IPlayUrls playUrl(Retrofit retrofit, String baseUrl, String html) {
        ...
        String match = JavaScriptUtil.match("var player_data=[\\u4e00-\\u9fa5\\(\\)\\{\\}\\[\\]\\\"\\w\\d`~！!@#$%^&*_\\-+=<>?:|,.\\\\ \\/;']+\"}", html, 0,16,0);
        if (!TextUtils.isEmpty(match)) {
            try {
                ...
                } else if (url.contains(".mp4") || url.contains(".wmv") || url.contains(".rm") || url.contains(".3gp") || url.contains(".avi")) {
                    urlMap.put(from, url);
                    urls.setPlayType(IVideoEpisode.PLAY_TYPE_VIDEO);
                    urls.setUrlType(IPlayUrls.URL_FILE);
                }
                if (urlMap.isEmpty()) {
                    urlMap.put(from, RetrofitManager.warpUrl(baseUrl, url));
